...
const TripsPage = () => {
  ...
  return(<div>
    ...
    <div className="filter-bar"><input type="date" className="form-control" style={{width:'auto'}} value={dateFilter} onChange={e=>setDateFilter(e.target.value)}/><select className="form-control" style={{width:'auto'}} value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option value="">All</option><option value="scheduled">Scheduled</option><option value="delayed">Delayed</option><option value="in_progress">In Progress</option><option value="completed">Completed</option><option value="missed">Not Started</option></select><select className="form-control" style={{width:'auto'}} value={timelinessFilter} onChange={e=>setTimelinessFilter(e.target.value)}><option value="">All timeliness</option><option value="not_started">⚠️ Not started</option><option value="delayed">⏱️ Delayed</option><option value="on_time">✅ On time</option><option value="upcoming">🕒 Upcoming</option></select><button className="btn btn-outline btn-sm" onClick={()=>{setDateFilter('');setStatusFilter('');setTimelinessFilter('');}}>Clear</button></div>
