...
export const vehicleAPI = { getAll: p => api.get('/vehicles',{params:p}), getById: id => api.get(`/vehicles/${id}`), getTripHistory: (id, days = 30) => api.get(`/vehicles/${id}/trip-history`, { params: { days } }), getStats: () => api.get('/vehicles/stats/summary'), create: d => api.post('/vehicles',d), update: (id,d) => api.put(`/vehicles/${id}`,d), delete: id => api.delete(`/vehicles/${id}`) };
...
export const routeAPI = { getAll: () => api.get('/routes'), getById: id => api.get(`/routes/${id}`), getTripHistory: (id, days = 30) => api.get(`/routes/${id}/trip-history`, { params: { days } }), create: d => api.post('/routes',d), update: (id,d) => api.put(`/routes/${id}`,d), delete: id => api.delete(`/routes/${id}`), suggestStudents: d => api.post('/routes/suggest-students',d) };
export const tripAPI = { getAll: p => api.get('/trips',{params:p}), getDetails: id => api.get(`/trips/${id}/details`), create: d => api.post('/trips',d), start: id => api.put(`/trips/${id}/start`), end: id => api.put(`/trips/${id}/end`), logAction: (id,d) => api.post(`/trips/${id}/log`,d), getLogs: id => api.get(`/trips/${id}/logs`), delete: id => api.delete(`/trips/${id}`) };
...
export const parentAPI = { getAll: () => api.get('/parents'), getById: id => api.get(`/parents/${id}`), create: d => api.post('/parents',d), update: (id,d) => api.put(`/parents/${id}`,d), delete: id => api.delete(`/parents/${id}`), reactivate: id => api.put(`/parents/${id}/reactivate`) };
